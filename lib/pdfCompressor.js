 ...
 * OPTIMIZADO PARA MANTENER MÁXIMA CALIDAD (Escala 2.0)
 */
 ...
 * Mantiene MÁXIMA CALIDAD visual mientras reduce el tamaño
 * 
 * @param {File|Blob} file - Archivo PDF a comprimir
 * @param {Object} options - Opciones de compresión
 * @param {number} options.quality - Calidad JPEG (0.1 a 1.0, default: 0.85)
 * @param {number} options.scale - Escala de renderizado (0.5 a 2.0, default: 2.0)
...
export async function compressPDF(file, options = {}) {
  const {
    quality = 0.85,  // MÁXIMA calidad por defecto
    scale = 2.0,     // Escala 2.0 para máxima resolución
    maxSizeMB = 9,
    onProgress = () => {}
  } = options
  ...
  try {
    ...
    // Calcular parámetros óptimos - PRIORIZANDO MÁXIMA CALIDAD
    let optimalScale = scale
    let optimalQuality = quality
    
    const sizeMB = originalSize / (1024 * 1024)
    
    // Parámetros para MÁXIMA CALIDAD con escala 2.0
    if (sizeMB > 80) { // > 80MB
      optimalScale = 1.5
      optimalQuality = 0.80
    } else if (sizeMB > 50) { // > 50MB (como PDFs de ~63MB)
      optimalScale = 2.0
      optimalQuality = 0.85
    ...
    console.log(`Parámetros de MÁXIMA CALIDAD: escala=${optimalScale}, calidad=${optimalQuality}`)
    ...
    if (compressedSize > maxSizeBytes && optimalQuality > 0.65) {
