...
export default function CobroPage() {
  ...
  const loadOrders = async () => {
    ...
    setOrdersPendientes(pendientes || [])
  ...
  // Función para marcar pedido como PENDIENTE (cobrar después)
  const handleMarcarPendiente = async (orderId) => {
    try {
      ...
      toast.success('Pedido movido a Pendientes')
      loadOrders()
    } catch (error) {
  ...
  // Función para regresar un pedido pendiente a "A Cobrar"
  const handleRegresarACobrar = async (orderId) => {
    try {
      const { error } = await supabase
        .from('orders')
      ...
      toast.success('Pedido regresado a "A Cobrar"')
      loadOrders()
    } catch (error) {
      console.error('Error regresando pedido:', error)
      toast.error('Error al regresar el pedido')
    }
  }
  ...
  const handleProcessPayment = async () => {
    ...
    // Validar montos de pago MIXTO
    if (paymentForm.metodo_pago === 'MIXTO') {
      const efectivo = parseFloat(paymentForm.monto_efectivo) || 0
      const tarjeta = parseFloat(paymentForm.monto_tarjeta) || 0
      const suma = efectivo + tarjeta
      const total = calculateFinalTotal()
      if (suma < total - 0.01) {
        toast.error(`Falta ${formatCurrency(total - suma)} para completar el pago`)
        return
      }
    }
    ...
    try {
      ...
      if (paymentForm.metodo_pago === 'MIXTO') {
        const ef = parseFloat(paymentForm.monto_efectivo) || 0
        const tj = parseFloat(paymentForm.monto_tarjeta) || 0
        metodoPagoFinal = `MIXTO (Ef: ${formatCurrency(ef)} + QR/Tar: ${formatCurrency(tj)})`
      }
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Tabs defaultValue="acobrar" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="pendientes" className="data-[state=active]:bg-amber-500 data-[state=active]:text-white">
              <Clock className="mr-1 h-4 w-4" /> Pendientes ({ordersPendientes.length})
            </TabsTrigger>
          ...
          {/* Pestaña PENDIENTES - Pedidos guardados para cobrar después */}
          <TabsContent value="pendientes">
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center">
              <Clock className="h-5 w-5 text-amber-600 mr-2" />
              <p className="text-sm text-amber-800">
                Pedidos marcados para cobrar más tarde. Podés cobrarlos cuando quieras.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {ordersPendientes.map(order => (
                <Card key={order.id} className="border-2 border-amber-300 shadow-md">
          ...
          <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
            ...
            {selectedOrder && (
              <div className="space-y-4">
                ...
                {/* Pago MIXTO - Inputs para efectivo y tarjeta/QR */}
                {paymentForm.metodo_pago === 'MIXTO' && (
                  <div className="space-y-3 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                    <div className="flex items-center space-x-2">
                      <CreditCard className="h-5 w-5 text-purple-600" />
                      <Label className="font-semibold text-purple-800">Dividir Pago</Label>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <Label className="text-sm flex items-center">
                          <Coins className="h-3 w-3 mr-1" /> Efectivo
                        </Label>
                        <Input
                          type="number"
                          value={paymentForm.monto_efectivo}
                          onChange={(e) => setPaymentForm({...paymentForm, monto_efectivo: e.target.value})}
                          placeholder="0"
                      ...
                      <div className="space-y-1">
                        <Label className="text-sm flex items-center">
                          <CreditCard className="h-3 w-3 mr-1" /> QR / Tarjeta
                        </Label>
                        <Input
                          type="number"
                          value={paymentForm.monto_tarjeta}
                          onChange={(e) => setPaymentForm({...paymentForm, monto_tarjeta: e.target.value})}
                          placeholder="0"
                          className="bg-white font-bold"
                        />
                      </div>
                    ...
                    <div className="bg-white p-3 rounded-lg border">
                      ...
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-semibold">
                          {formatCurrency((parseFloat(paymentForm.monto_efectivo) || 0) + (parseFloat(paymentForm.monto_tarjeta) || 0))}
                        </span>
                      </div>
                      {(() => {
                        const suma = (parseFloat(paymentForm.monto_efectivo) || 0) + (parseFloat(paymentForm.monto_tarjeta) || 0)
                        const total = calculateFinalTotal()
                        const diff = suma - total
                        if (Math.abs(diff) < 0.01) {
                          return (
                            <div className="flex justify-between text-sm pt-2 border-t text-green-600 font-bold">
                              <span>✓ Coincide</span>
                              <span>OK</span>
                            </div>
                          )
                          ...
                          return (
                            <div className="flex justify-between text-sm pt-2 border-t text-red-600 font-bold">
                              <span>Falta:</span>
                          ...
                          return (
                            <div className="flex justify-between text-sm pt-2 border-t text-amber-600 font-bold">
                              <span>Sobra (vuelto):</span>
